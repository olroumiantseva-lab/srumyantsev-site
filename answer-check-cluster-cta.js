(() => {
  ...
  const configs = {
    ...
    // Ступень 2 — третий маршрут «Система работы».
    // Архив документов сознательно не включён: там логичнее продукт разбора документа.
    '/rezhimy-i-modeli-neyroseti/': {
      mode: 'soft', source: 'guide-modes-models',
      title: 'Режим влияет на качество, но не гарантирует правильность',
      text: 'Поиск и глубокое рассуждение снижают часть ошибок, но даже сильный режим может неверно понять источник или сделать лишний вывод. Если ответ влияет на решение, его можно проверить отдельно.',
      terms: ['поиск', 'исследован', 'режим', 'модел']
    },
    '/svoya-biblioteka-promptov/': {
      mode: 'inline', source: 'guide-prompt-library',
      text: 'Хороший промпт стоит сохранять вместе с правилом проверки результата. Если шаблон используется для фактов, денег, документов или решений, важный готовый ответ можно отдельно проверить перед повторным использованием.',
      terms: ['проверен', 'шаблон', 'библиотек', 'повтор']
    },
    '/lichnaya-sistema-doveriya-neyroseti/': {
      mode: 'full', source: 'guide-trust-system',
      title: 'Для важных ответов добавьте отдельную проверку',
      text: 'Если по вашей системе ответ попал в зону повышенного риска, не проверяйте всё вручную. Вставьте готовый ответ — сервис выделит спорные утверждения, сверит факты и источники и покажет, что осталось проверить самостоятельно.',
      terms: ['уровень проверки', 'система доверия', 'проверять всегда', 'быстрая сверка']
    },
    '/neyroset-v-rabote-gde-ekonomit-chasy/': {
      mode: 'soft', source: 'guide-ai-at-work',
      title: 'Рабочий черновик можно принять быстро. Фактический вывод — лучше проверить',
      text: 'Если нейросеть пишет письмо или структурирует заметки, отдельная проверка часто не нужна. Но вывод по договору, рынку, цифрам, правилам или клиентской ситуации лучше проверить до того, как он уйдёт дальше по рабочему процессу.',
      terms: ['где экономит', 'провер', 'ошиб', 'контрол']
    },
    '/uchitsya-s-neyrosetyu-yazyk-professiya-tehnika/': {
      mode: 'inline', source: 'guide-learning-ai',
      text: 'В обучении особенно легко запомнить красиво объяснённую ошибку. Если нейросеть дала конкретный факт, правило, формулу или техническое объяснение, которое вы собираетесь учить, сначала проверьте сам ответ.',
      terms: ['провер', 'пониман', 'ошиб', 'факт']
    }
  };
