(() => {
  const root = document.getElementById('ai-helper-wizard'); if (!root) return;
  const params = new URLSearchParams(location.search); const orderId = params.get('order_id') || ''; const token = params.get('token') || '';
  const config = window.__SUPABASE_CONFIG__ || {}; const base = config.url || 'https://vhssshjcrirsuiijolwq.supabase.co';
  ...
  const validate = i => { const s=state(); if(i===0&&!s.scenario)return'Выберите одну основную задачу.'; if(i===0&&s.task_description.length<12)return'Опишите задачу хотя бы одним предложением.'; if(i===1&&(!s.user_role||!s.input_type||!s.desired_outcome))return'Заполните роль, входные данные и желаемый результат.'; if(i===5&&s.test_input.length<12)return'Добавьте реальную мини-задачу для проверки.'; return''; };
