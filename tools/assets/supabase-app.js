(() => {
  ...
  async function loadResult(client) {
    ...
    await renderMessages(client, item.id);
    setupFollowup(client, item.id, item.followups_used);
  }
  ...
  async function renderMessages(client, sessionId) {
    const thread = byId('followup-thread'); thread.replaceChildren();
    const { data } = await client.from('followup_messages').select('role,content,created_at').eq('document_session_id', sessionId).order('created_at');
    for (const item of data ?? []) { const node = document.createElement('div'); node.className = `message ${item.role === 'user' ? 'message-user' : 'message-answer'}`; node.textContent = item.content; thread.append(node); }
  }

  function setupFollowup(client, sessionId, initialUsed) {
    let used = initialUsed;
    const form = byId('followup-form'); const input = byId('followup-input'); const button = form.querySelector('button');
    const update = () => {
      text(byId('followup-remaining'), `Осталось уточнений: ${3 - used} из 3. Они не расходуют разборы.`);
      const limited = used >= 3; input.disabled = limited; button.disabled = limited; byId('followup-limit').classList.toggle('hidden', !limited);
    };
    update();
    form.addEventListener('submit', async (event) => {
      event.preventDefault(); const question = input.value.trim(); if (!question || used >= 3) return;
      button.disabled = true;
      const { data, error } = await client.functions.invoke('mock-followup', { body: { session_id: sessionId, request_id: crypto.randomUUID(), question } });
      if (error) { text(byId('followup-error'), errorMessage(error, 'Не удалось ответить. Попробуйте ещё раз.')); button.disabled = false; return; }
      input.value = ''; used = data.followups_used; await renderMessages(client, sessionId); update(); if (used < 3) input.focus();
    });
  }
