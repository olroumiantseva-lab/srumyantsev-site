...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: order, error: orderError } = await admin
      .from("payment_orders")
      .select("id,email,amount_kopecks,status,user_id,product_id,source_site")
      .eq("id", invId)
      .single();

    const expectedOutSum = order?.amount_kopecks ? kopecksToOutSum(order.amount_kopecks) : null;
    if (orderError || !order || !expectedOutSum || outSum !== expectedOutSum) {
      logPayment("callback_rejected", {
        order_id: logInvId,
    ...
    const { error: rpcError } = await admin.rpc("complete_product_payment", {
      p_order_id: Number(invId),
      p_user_id: userId,
    });
    if (rpcError) throw rpcError;

    logPayment("callback_completed", {
      order_id: logInvId,
