(() => {
  const shorts = {
    ...
    '/kak-ponyat-analizy-i-zaklyuchenie-vracha/': { id:'XDLaVv65Mc4', title:'Как я попросил нейросеть объяснить медицинские термины', caption:'Показываю простой сценарий: дать нейросети непонятные медицинские термины и попросить объяснить их обычным языком — без попытки ставить диагноз.' },
    ...
    '/kak-podobrat-film-ili-knigu-s-pomoshchyu-neyroseti/': { id:'LsDgOUzKuC0', title:'Как подобрать фильм на вечер с помощью нейросети', caption:'Показываю, как задать несколько нормальных критериев вместо бесконечного листания каталогов и быстро получить подходящие варианты.' },
    '/kratkiy-pereskaz-video-youtube-neyrosetyu/': { id:'NKDKSvOP0Ug', title:'Полтора часа видео — за пять минут', caption:'Реальный бытовой сценарий: получить короткое содержание длинного вебинара, а потом открыть только нужные фрагменты по тайм-кодам.' },
    '/pochemu-neyroset-daet-skuchnye-otvety/': { id:'6Ggt9VpHfBE', title:'Как сделать ответы нейросети менее скучными', caption:'Показываю, что проблема часто не в самой нейросети, а в слишком общем запросе — несколько уточнений заметно меняют ответ.' },
    '/kak-podobrat-podarok-s-pomoshchyu-neyroseti/': { id:'9HUOcht54lI', title:'Подарок человеку, у которого всё есть', caption:'Живой пример: возраст, интересы, бюджет и несколько ограничений — и нейросеть помогает быстро получить не банальный список идей.' },
    '/dve-neyroseti-v-pare-vtoroe-mnenie/': { id:'pjJoR4cuOHI', title:'Один вопрос — два разных ответа', caption:'Задал двум нейросетям один и тот же вопрос про гарантии и получил разные ответы. Хороший пример того, зачем иногда нужно второе мнение.' },
    '/pochemu-neyroset-otkazyvaetsya-otvechat/': { id:'aQN_p3aUhK0', title:'Нейросеть отказалась отвечать — что изменить в запросе', caption:'На медицинском примере показываю разницу между просьбой дать персональный совет и просьбой объяснить информацию из инструкции.' }
  };
