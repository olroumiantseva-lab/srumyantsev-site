...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 17, "2-route": 6, "2-practical": 11 };
const practicumUrls = [
...
assert.equal(registry.length, 53, "Registry must contain 53 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 53, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 53, "Catalog HTML must contain 53 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 53, "Catalog HTML URLs must be unique");
...
console.log("PASS: 53 guides, routes 6/7/6, practical 6/17/11, canonicals and links intact");
