...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 22, "2-route": 6, "2-practical": 12 };
const practicumUrls = [
...
assert.equal(registry.length, 59, "Registry must contain 59 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 59, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 59, "Catalog HTML must contain 59 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 59, "Catalog HTML URLs must be unique");
...
console.log("PASS: 59 guides, routes 6/7/6, practical 6/22/12, canonicals and links intact");
