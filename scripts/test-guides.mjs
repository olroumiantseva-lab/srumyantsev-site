...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 8, "2-route": 6, "2-practical": 2 };

assert.equal(registry.length, 35, "Registry must contain 35 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 35, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 35, "Catalog HTML must contain 35 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 35, "Catalog HTML URLs must be unique");
...
console.log("PASS: 35 guides, routes 6/7/6, practical 6/8/2, canonicals and links intact");
