...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 11, "2-route": 6, "2-practical": 5 };

assert.equal(registry.length, 41, "Registry must contain 41 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 41, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 41, "Catalog HTML must contain 41 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 41, "Catalog HTML URLs must be unique");
...
console.log("PASS: 41 guides, routes 6/7/6, practical 6/11/5, canonicals and links intact");
