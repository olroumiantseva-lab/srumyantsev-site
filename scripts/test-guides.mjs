...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 21, "2-route": 6, "2-practical": 12 };
const practicumUrls = [
...
assert.equal(registry.length, 58, "Registry must contain 58 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 58, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 58, "Catalog HTML must contain 58 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 58, "Catalog HTML URLs must be unique");
...
console.log("PASS: 58 guides, routes 6/7/6, practical 6/21/12, canonicals and links intact");
