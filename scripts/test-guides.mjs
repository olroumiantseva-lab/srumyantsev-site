...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 17, "2-route": 6, "2-practical": 10 };
const practicumUrls = [
...
assert.equal(registry.length, 52, "Registry must contain 52 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 52, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 52, "Catalog HTML must contain 52 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 52, "Catalog HTML URLs must be unique");
...
console.log("PASS: 52 guides, routes 6/7/6, practical 6/17/10, canonicals and links intact");
