...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 10, "2-route": 6, "2-practical": 4 };

assert.equal(registry.length, 39, "Registry must contain 39 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 39, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 39, "Catalog HTML must contain 39 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 39, "Catalog HTML URLs must be unique");
...
console.log("PASS: 39 guides, routes 6/7/6, practical 6/10/4, canonicals and links intact");
