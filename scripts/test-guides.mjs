...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 15, "2-route": 6, "2-practical": 9 };
const practicumUrls = [
...
assert.equal(registry.length, 49, "Registry must contain 49 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 49, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 49, "Catalog HTML must contain 49 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 49, "Catalog HTML URLs must be unique");
...
console.log("PASS: 49 guides, routes 6/7/6, practical 6/15/9, canonicals and links intact");
