...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 11, "2-route": 6, "2-practical": 4 };

assert.equal(registry.length, 40, "Registry must contain 40 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 40, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 40, "Catalog HTML must contain 40 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 40, "Catalog HTML URLs must be unique");
...
console.log("PASS: 40 guides, routes 6/7/6, practical 6/11/4, canonicals and links intact");
