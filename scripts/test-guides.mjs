...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 8, "2-route": 6, "2-practical": 1 };

assert.equal(registry.length, 34, "Registry must contain 34 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 34, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 34, "Catalog HTML must contain 34 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 34, "Catalog HTML URLs must be unique");
...
console.log("PASS: 34 guides, routes 6/7/6, practical 6/8/1, canonicals and links intact");
