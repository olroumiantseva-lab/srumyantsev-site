...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 13, "2-route": 6, "2-practical": 8 };
const practicumUrls = [
...
assert.equal(registry.length, 46, "Registry must contain 46 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 46, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 46, "Catalog HTML must contain 46 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 46, "Catalog HTML URLs must be unique");
...
console.log("PASS: 46 guides, routes 6/7/6, practical 6/13/8, canonicals and links intact");
