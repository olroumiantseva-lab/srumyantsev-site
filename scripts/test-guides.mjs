...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 20, "2-route": 6, "2-practical": 12 };
const practicumUrls = [
...
assert.equal(registry.length, 57, "Registry must contain 57 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 57, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 57, "Catalog HTML must contain 57 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 57, "Catalog HTML URLs must be unique");
...
console.log("PASS: 57 guides, routes 6/7/6, practical 6/20/12, canonicals and links intact");
