...
const expectedCounts = { "0-route": 6, "0-practical": 6, "1-route": 7, "1-practical": 18, "2-route": 6, "2-practical": 11 };
const practicumUrls = [
...
assert.equal(registry.length, 54, "Registry must contain 54 guides");
assert.equal(new Set(registry.map((guide) => guide.url)).size, 54, "Guide URLs must be unique");
...
assert.equal(catalogCards.length, 54, "Catalog HTML must contain 54 cards without JavaScript");
...
assert.equal(new Set(catalogUrls).size, 54, "Catalog HTML URLs must be unique");
...
console.log("PASS: 54 guides, routes 6/7/6, practical 6/18/11, canonicals and links intact");
