...
6. Оплатить тестовый заказ и проверить:
   ...
   - одна `credit_transactions` типа `purchase` на +10;
   - повторный callback не добавляет ещё +10.
   - для нового email абсолютный баланс после оплаты может быть 11: +1 `signup_grant` создаётся отдельно при первом создании пользователя. Критерий smoke — именно одна покупочная транзакция +10 и отсутствие повторного начисления.
